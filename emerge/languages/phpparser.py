"""
Contains the implementation of the PHP language parser and a relevant keyword enum.
"""
...
class PHPParser(AbstractParser, ParsingMixin):
    def __init__(self):
    ...
    def parser_name(cls) -> str:
        return Parser.PHP_PARSER.name

    @classmethod
    def language_type(cls) -> str:
        return LanguageType.PHP.name
    ...
    def generate_file_result_from_analysis(self, analysis, *, file_name: str, full_file_path: str, file_content: str) -> None:
        ...
        file_result = FileResult.create_file_result(
            ...
            scanned_language=LanguageType.PHP,
    ...
    def after_generated_file_results(self, analysis) -> None:
        # curate dependencies from the first scan PHP format that actually exists, to match the real dependencies
        ...
        for _, result in filtered_results.items():
            ...
            for dependency in result.scanned_import_dependencies:
                ...
                haystack = dependency.replace("\\", "/") + ".php"
    ...
    def create_unique_entity_name(self, entity: AbstractEntityResult) -> None:
        if entity.module_name:
            entity.unique_name = entity.module_name + PHPParsingKeyword.BACKSLASH.value + entity.entity_name
        else:
    ...
    def generate_entity_results_from_analysis(self, analysis):
        ...
        for _, result in filtered_results.items():
            ...
            entity_keywords: List[str] = [
                PHPParsingKeyword.CLASS.value,
                PHPParsingKeyword.INTERFACE.value,
                PHPParsingKeyword.TRAIT.value
            ]
            entity_name = pp.Word(pp.alphanums + CoreParsingKeyword.UNDERSCORE.value)

            # Match: [abstract|final] class|interface|trait Name [extends Parent] [implements Interface]
            match_expression = pp.Optional(pp.Keyword(PHPParsingKeyword.ABSTRACT.value) | pp.Keyword(PHPParsingKeyword.FINAL.value)) + \
                (pp.Keyword(PHPParsingKeyword.CLASS.value) | pp.Keyword(PHPParsingKeyword.INTERFACE.value) | pp.Keyword(PHPParsingKeyword.TRAIT.value)) + \
                entity_name.setResultsName(CoreParsingKeyword.ENTITY_NAME.value) + \
                pp.Optional(pp.Keyword(PHPParsingKeyword.EXTENDS.value) +
                            entity_name.setResultsName(CoreParsingKeyword.INHERITED_ENTITY_NAME.value)) + \
                pp.SkipTo(pp.FollowedBy(PHPParsingKeyword.OPEN_SCOPE.value))

            comment_keywords: Dict[str, str] = {
                CoreParsingKeyword.LINE_COMMENT.value: PHPParsingKeyword.INLINE_COMMENT.value,
                CoreParsingKeyword.START_BLOCK_COMMENT.value: PHPParsingKeyword.START_BLOCK_COMMENT.value,
                CoreParsingKeyword.STOP_BLOCK_COMMENT.value: PHPParsingKeyword.STOP_BLOCK_COMMENT.value
            }
    ...
    def _add_imports_to_entity_result(self, entity_result: EntityResult):
        ...
        for scanned_import in entity_result.parent_file_result.scanned_import_dependencies:
            # Get last component of import (e.g., User from App\Models\User)
            last_component_of_import = scanned_import.split(PHPParsingKeyword.BACKSLASH.value)[-1]
    ...
    def _add_imports_to_result(self, result: FileResult, analysis):
        ...
        source_string_no_comments = self._filter_source_tokens_without_comments(
            ...
            PHPParsingKeyword.INLINE_COMMENT.value,
            PHPParsingKeyword.START_BLOCK_COMMENT.value,
            PHPParsingKeyword.STOP_BLOCK_COMMENT.value
        )
        ...
        for _, obj, following in self._gen_word_read_ahead(filtered_list_no_comments):
            if obj == PHPParsingKeyword.USE.value:
                read_ahead_string = self.create_read_ahead_string(obj, following)

                # Match use statements like: use App \ Models \ User ; -> App\Models\User
                namespace_part = pp.Word(pp.alphanums + CoreParsingKeyword.UNDERSCORE.value)
                imported_entity_name = pp.Combine(namespace_part + pp.ZeroOrMore(pp.Literal("\\") + namespace_part), adjacent=False, joinString='')
                expression_to_match = pp.Keyword(PHPParsingKeyword.USE.value) + \
                    imported_entity_name.setResultsName(CoreParsingKeyword.IMPORT_ENTITY_NAME.value) + \
                    pp.Optional(pp.Keyword("as") + pp.Word(pp.alphanums + CoreParsingKeyword.UNDERSCORE.value)) + \
    ...
    def _add_namespace_to_result(self, result: FileResult):
        LOGGER.debug(f'extracting namespace from base result {result.scanned_file_name}...')
        list_of_words = result.scanned_tokens
        for _, obj, following in self._gen_word_read_ahead(list_of_words):
            if obj == PHPParsingKeyword.NAMESPACE.value:
                read_ahead_string = self.create_read_ahead_string(obj, following)

                # Match namespace with backslash separators: App \ Models -> App\Models
                namespace_part = pp.Word(pp.alphanums + CoreParsingKeyword.UNDERSCORE.value)
                namespace_name = pp.Combine(namespace_part + pp.ZeroOrMore(pp.Literal("\\") + namespace_part), adjacent=False, joinString='')
                expression_to_match = pp.Keyword(PHPParsingKeyword.NAMESPACE.value) + namespace_name.setResultsName(PHPParsingKeyword.NAMESPACE_NAME.value) + \
                    pp.FollowedBy(CoreParsingKeyword.SEMICOLON.value)
    ...
    def _add_inheritance_to_entity_result(self, result: AbstractEntityResult):
        ...
        list_of_words = result.scanned_tokens
        for _, obj, following in self._gen_word_read_ahead(list_of_words):
            if obj in [PHPParsingKeyword.CLASS.value, PHPParsingKeyword.INTERFACE.value, PHPParsingKeyword.TRAIT.value]:
                read_ahead_string = self.create_read_ahead_string(obj, following)

                entity_name = pp.Word(pp.alphanums + CoreParsingKeyword.UNDERSCORE.value)
                expression_to_match = pp.Keyword(obj) + entity_name.setResultsName(CoreParsingKeyword.ENTITY_NAME.value) + \
                    pp.Optional(pp.Keyword(PHPParsingKeyword.EXTENDS.value) + entity_name.setResultsName(CoreParsingKeyword.INHERITED_ENTITY_NAME.value)) + \
                    pp.SkipTo(pp.FollowedBy(PHPParsingKeyword.OPEN_SCOPE.value))
